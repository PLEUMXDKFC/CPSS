...
function CheckplanPVSS() {
    ...
    const processCredits = () => {
        ...
        const credits = {
            ...
            "รายวิชาปรับพื้นฐาน": { terms: {1:0,2:0,3:0,4:0,'S':0,6:0}, total: 0 }
        };
        ...
        filteredData.forEach(item => {
            ...
            // Convert term to the correct key
            const termKey = term === 'summer' ? 'S' : term;
    
            if (subject_category === "รายวิชาปรับพื้นฐาน") {
                credits["รายวิชาปรับพื้นฐาน"].terms[termKey] += numericCredit;
                credits["รายวิชาปรับพื้นฐาน"].total += numericCredit;
            }
            else if (subject_category === "3.หมวดวิชาเลือกเสรี") {
                credits[subject_category].terms[termKey] += numericCredit;
                credits[subject_category].total += numericCredit;
            } 
            else if (credits[subject_category]?.[subject_groups]) {
                credits[subject_category][subject_groups].terms[termKey] += numericCredit;
...
const getGrandTotal = () => {
    let total = 0;
    const terms = [1, 2, 3, 4, 'S', 6]; // Include all terms including summer
        ...
        <div className="flex h-screen">
          ...
          <div className="flex-1 ml-65 p-6 overflow-auto">
                            ...
                            <td colSpan="8" className="border  align-top">
                                ...
                                <p className="pt-2">
                                    ...
                                    <label id="semester-info1">
                                        สำหรับนักเรียนรหัส {studentId} {groupNames.length > 1 && selectedGroup && `ก.${selectedGroup}`} ม.6
                                    </label>
                                </p>
                ...
                <tbody id="table_result">
                    ...
                    <tr>
                    <td className="border px-2 py-1 text-left border-t-0 border-b-0 font-bold">รายวิชาปรับพื้นฐาน</td>
                    <td className="border px-2 py-1 border-t-0">
                        {credits["รายวิชาปรับพื้นฐาน"].terms[1] > 0 
                            ? credits["รายวิชาปรับพื้นฐาน"].terms[1] 
                            : ''}
                    </td>
                    <td className="border px-2 py-1 border-t-0">
                        {credits["รายวิชาปรับพื้นฐาน"].terms[2] > 0 
                            ? credits["รายวิชาปรับพื้นฐาน"].terms[2] 
                            : ''}
                    </td>
                    <td className="border px-2 py-1 border-t-0">
                        {credits["รายวิชาปรับพื้นฐาน"].terms[3] > 0 
                            ? credits["รายวิชาปรับพื้นฐาน"].terms[3] 
                            : ''}
                    </td>
                    <td className="border px-2 py-1 border-t-0">
                        {credits["รายวิชาปรับพื้นฐาน"].terms[4] > 0 
                            ? credits["รายวิชาปรับพื้นฐาน"].terms[4] 
                            : ''}
                    </td>
                    <td className="border px-2 py-1 border-t-0">
                        {credits["รายวิชาปรับพื้นฐาน"].terms['S'] > 0 
                            ? credits["รายวิชาปรับพื้นฐาน"].terms['S'] 
                            : ''}
                    </td>
                    <td className="border px-2 py-1 border-t-0">
                        {credits["รายวิชาปรับพื้นฐาน"].terms[6] > 0 
                            ? credits["รายวิชาปรับพื้นฐาน"].terms[6] 
                            : ''}
                    </td>
                    <td className="border px-2 py-1 border-t-0">
                        {credits["รายวิชาปรับพื้นฐาน"].total > 0 
                            ? credits["รายวิชาปรับพื้นฐาน"].total 
                            : '0'}
                    </td>
                    <td className="border px-2 py-1 text-center font-bold" id="text8">ไม่น้อยกว่า 15 </td>
                    </tr>
                    <tr className="font-bold">
                    ...
                    <td className="border px-2 py-1 border-t-0">
                    {courseData
                        .filter(item => item.subject.subject_category === "4.กิจกรรมเสริมหลักสูตร")
                    ...
                    <td className="border px-2 py-1 text-center" id="text9">ไม่น้อยกว่า 95 นก.</td>
                    </tr>
                </tbody>
                </table>
